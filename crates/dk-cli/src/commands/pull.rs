...
pub fn run(remote: Option<String>, branch: Option<String>) -> Result<()> {
    ...
    let workdir = repo
        .workdir()
        .context("cannot pull into a bare repository")?
        .to_path_buf();
    ...
    // Inherit stderr so git pull progress (fetching objects, resolving
    // deltas, updating refs) streams to the terminal in real time.
    ...
    // Print stdout (merge/rebase output)
