...
use dk_engine::parser::langs::python::PythonConfig;
use dk_engine::parser::LanguageParser;
use std::path::Path;

#[test]
fn test_extract_symbols() {
    let parser = QueryDrivenParser::new(Box::new(PythonConfig)).unwrap();
    let source = br#"
MAX_RETRIES = 3
_internal_flag = True
...
class ApiController:
    pass
"#;
    let analysis = parser.parse_file(source, Path::new("auth.py")).unwrap();
...
"#;
    let analysis = parser.parse_file(source, Path::new("main.py")).unwrap();
...
"#;
    let analysis = parser.parse_file(source, Path::new("app.py")).unwrap();
...
def create_config(path):
    ...
    return cfg
"#;
    let analysis = parser.parse_file(source, Path::new("config.py")).unwrap();
...
def protected():
    pass
"#;
    let analysis = parser.parse_file(source, Path::new("views.py")).unwrap();
