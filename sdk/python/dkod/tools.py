"""LLM tool descriptors and dispatch for the dkod Agent Protocol.

Provides :func:`dkod_tools` to generate Claude-compatible tool definitions
with ``allowed_callers`` for programmatic tool calling, and
:func:`dispatch_tool` to execute tool calls against a
:class:`~dkod.session.DkodSession`.
"""
...
from dkod.models import Change, ChangeType, ContextDepth
...
def dispatch_tool(session: DkodSession, name: str, input: dict) -> str:
    """Execute a tool call against *session* and return JSON.

    Parameters
    ----------
    session:
        An active :class:`~dkod.session.DkodSession`.
    name:
